...
plt.plot(PAINT_POINTS[0], 1 * np.tan(PAINT_POINTS[0]) + 1, c='#74BCFF', lw=3, label='upper bound')
plt.plot(PAINT_POINTS[0], 1 * np.tan(PAINT_POINTS[0]) + 0, c='#FF9359', lw=3, label='lower bound')
...
def artist_works():     # painting from the famous artist (real target)
    ...
    paintings = 1 * np.tan(PAINT_POINTS) + (a-1)
    return paintings
...
with tf.variable_scope('Discriminator'):
    ...
    prob_artist1 = tf.layers.dense(D_l1, 1, tf.nn.sigmoid, name='out', reuse=True)  # probability that the art work is made by generator
...
for step in range(7000):
    ...
    G_paintings, pa0, pa1, Dl = sess.run([G_out, prob_artist0, prob_artist1, D_loss, train_D, train_G],    # train and get results
                                    {G_in: G_ideas, real_art: artist_paintings})[:4]

    if step % 50 == 0 and step > 5000:  # plotting
        plt.cla()
        plt.plot(PAINT_POINTS[0], G_paintings[0], c='#4AD631', lw=3, label='Generated painting',)
        plt.plot(PAINT_POINTS[0], 1 * np.tan(PAINT_POINTS[0]) + 1, c='#74BCFF', lw=3, label='upper bound')
        plt.plot(PAINT_POINTS[0], 1 * np.tan(PAINT_POINTS[0]) + 0, c='#FF9359', lw=3, label='lower bound')
        plt.text(-.5, 2.6, 'D1 accuracy=%.2f (0.5 for D to converge)' % pa0.mean(), fontdict={'size': 15})
